...
本项目是 OpenAI [`openai/codex-plugin-cc`](https://github.com/openai/codex-plugin-cc)
（Apache-2.0）的**派生作品**。Claude Code 宿主层、作业/运行器状态机以及整体插件结构均沿用自该项目，
原始设计的功劳归属原作者。

引擎层被重写，把底层编码引擎从 Codex 换成 ZCode（通过逆向 `zcode app-server` 所说的 ZCode Protocol 实现）。
完整的派生作品归属说明见 `NOTICE`（依 Apache 许可证要求）。

本派生作品由 **ZCode**（使用 Z.ai 的 **GLM-5.2** 模型）编写，并经人工辅助打磨。

## 许可证

Apache-2.0（与上游 `openai/codex-plugin-cc` 相同）。见 `LICENSE` 和 `NOTICE`。
