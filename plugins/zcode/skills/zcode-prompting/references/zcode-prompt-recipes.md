# ZCode Prompt Recipes

Use these as starting templates for ZCode task prompts or other ZCode prompt construction.
Copy the smallest recipe that fits the task, then trim anything you do not need.
In `zcode:zcode-rescue`, run diagnosis and fix-oriented recipes in write mode by default unless the user explicitly asked for read-only behavior.
