...
Prompt ZCode like an operator, not a collaborator. Keep prompts compact and block-structured with XML tags. State the task, the output contract, the follow-through defaults, and the small set of extra constraints that matter.

Core rules:
- Prefer one clear task per ZCode run. Split unrelated asks into separate runs.
- Tell ZCode what done looks like. Do not assume it will infer the desired end state.
...
- `<default_follow_through_policy>`: what ZCode should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so ZCode stays narrow and avoids unrelated refactors.
...
- Use `task --resume-last` for follow-up instructions on the same ZCode session. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
...
- Ask ZCode for brief, outcome-based progress updates only when the task is long-running or tool-heavy.
...
3. Decide whether ZCode should keep going by default or stop for missing high-risk details.
...
Concrete end-to-end templates live in [references/zcode-prompt-recipes.md](references/zcode-prompt-recipes.md).
Common failure modes to avoid live in [references/zcode-prompt-antipatterns.md](references/zcode-prompt-antipatterns.md).
