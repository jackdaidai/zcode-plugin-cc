...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No job found for "${reference}". Run /zcode:status to list known jobs.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No job found for "${reference}". Run /zcode:status to inspect known jobs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check /zcode:status and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished job found for "${reference}". Run /zcode:status to inspect active jobs.`);
  }
...
export function resolveCancelableJob(cwd, reference, options = {}) {
  ...
  if (sessionScopedActiveJobs.length > 1) {
    throw new Error("Multiple ZCode jobs are active. Pass a job id to /zcode:cancel.");
  }

  if (getCurrentSessionId(options)) {
    throw new Error("No active ZCode jobs to cancel for this session.");
  }

  throw new Error("No active ZCode jobs to cancel.");
}
