...
- If the raw arguments include `--background`, do not ask. Run the review in a ZCode background task.
...
- The companion script parses `--wait` and `--background`, but ZCode's `Bash(..., run_in_background: true)` is what actually detaches the run.
